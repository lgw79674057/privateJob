...
dialog.error = function(options) {
	  ...
	  html.push('<div class="am-modal-dialog">');
	  html.push('<div class="am-modal-hd">' + options.title + '</div>');
	  html.push('<div class="am-modal-bd"><i class="am-icon-frown-o am-icon-lg" style="color:#dd514c">&nbsp;</i>' + options.content + '</div>');
	  html.push('<div class="am-modal-footer"><span class="am-modal-btn">确定<span id="timeout"></span></span></div>');
	  html.push('</div>');
	  html.push('</div>');
	  var window=$(html.join('')).appendTo('body').modal();
	  if(options.timeOut>0){
		  var i=options.timeOut/1000;
		  var span=window.find("#timeout");
		  span.html("&nbsp;("+i+"S)")
	  ...
	  return window.on('closed.modal.amui', function() {options.onConfirm();$(this).remove();});
};


dialog.success = function(options) {
	  options = options || {};
	  options.title = options.title || '提示';
	  options.content = options.content || '提示内容';
	  options.onConfirm = options.onConfirm || function() {
	  ...
	  html.push('<div class="am-modal-dialog">');
	  html.push('<div class="am-modal-hd">' + options.title + '</div>');
	  html.push('<div class="am-modal-bd"><i class="am-icon-smile-o am-icon-lg" style="color:#5eb95e">&nbsp;</i>' + options.content + '</div>');
	  html.push('<div class="am-modal-footer"><span class="am-modal-btn">确定<span id="timeout"></span></span></div>');
	  html.push('</div>');
	  html.push('</div>');
	  var window=$(html.join('')).appendTo('body').modal();
...
dialog.confirm = function(options) {
  ...
  html.push('<span class="am-modal-btn" data-am-modal-cancel>'+options.cancel+'</span>');
  html.push('<span class="am-modal-btn" data-am-modal-confirm>'+options.confirm+'</span>');
  html.push('</div>');
  html.push('</div>');
  html.push('</div>');
...
dialog.prompt = function(options) {
	  ...
	  html.push('<div class="am-modal-dialog">');
	  html.push('<div class="am-modal-hd">' + options.title + '</div>');
	  html.push('<div class="am-modal-bd">' + options.content + '<input type="text" placeholder="'+options.placeholder+'" class="am-modal-prompt-input"></div>');
	  html.push('<div class="am-modal-footer">');
